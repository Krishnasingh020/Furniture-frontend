...
async function testAPIConnection() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn('[API] Backend server returned error status:', response.status);
      return false;
    }
  } catch (error) {
    console.error('[API] Backend server is not reachable:', error.message);
    console.error('[API] Make sure the backend server is running at', API_BASE_URL);
    return false;
  }
}
...
async function fetchProducts(category = null) {
  ...
  try {
    ...
    console.log('[API] fetchProducts result length:', Array.isArray(data) ? data.length : 'N/A');
    return data;
  } catch (error) {
...
function getCurrentCategorySlug() {
  ...
  if (match && match[1]) {
    console.log('[getCurrentCategorySlug] slug from filename fallback:', match[1]);
    return decodeURIComponent(match[1]);
  }
...
function updateReviews(reviews) {
  ...
  reviews.forEach((review, index) => {
    ...
    reviewItem.className = `review ${index % 2 === 0 ? 'even' : 'odd'} ${index % 2 === 0 ? 'thread-even' : 'thread-odd'
      } depth-1`;
  ...
  if (summaryContainer) {
    ...
    summaryContainer.innerHTML = `
      ...
      <p style="margin-top: 0.5em; color: #646360;">Based on ${reviews.length} review${reviews.length !== 1 ? 's' : ''
      }</p>
    `;
  }
}
...
function getPageFilename() {
  ...
  console.log('[getPageFilename] Detected filename:', filename, 'from URL:', window.location.href);
  return filename;
}
...
async function initializeProductPage() {
  try {
    ...
    console.log('[initializeProductPage] Params:', { productId, productSlug, articleNumber, productParam });
    ...
    if (productMatch && productMatch[1]) {
      console.log('[initializeProductPage] Extracted slug from path:', productMatch[1]);
      productData = await fetchProductBySlug(productMatch[1]);
    } else if (articleNumber) {
      console.log('[initializeProductPage] Fetching by article:', articleNumber);
      ...
      console.log('[initializeProductPage] Fetching by product param:', productParam);
    ...
    if (!productData) {
      ...
      console.log('[initializeProductPage] Fallback identifier:', pageIdentifier);
      ...
      if (pageIdentifier) {
        if (pageIdentifier.articleNumber) {
          ...
          console.log('[initializeProductPage] Identifier is a category, skipping product fetch');
        }
      } else {
        ...
        if (match && match[1]) {
          ...
          const nonProductSlugs = ['decor', 'mirrors', 'rugs', 'tact', 'tact-mirror'];
          if (!nonProductSlugs.includes(potentialSlug)) {
            console.log('[initializeProductPage] Extracted slug from filename:', potentialSlug);
    ...
    if (productData) {
      console.log('[initializeProductPage] Product data found:', productData.name);
...
async function initializeCategoryPage() {
  try {
    ...
    if (!categoryTitleEl && !categoryDescEl) {
      console.log('[initializeCategoryPage] No category elements found, skipping');
      return;
    }
    ...
    console.log('[initializeCategoryPage] Slug:', slug, ', isCategory:', isCategory);
    ...
    try {
      ...
      console.log('[initializeCategoryPage] Attempting to fetch as category:', slug);
      try {
        rawData = await fetchCategoryBySlug(slug);
        categoryObj = rawData.category || rawData;
        isCategory = true;
      } catch (err) {
        console.log('[initializeCategoryPage] Not a category, trying subcategory:', slug);
      ...
      console.error('[initializeCategoryPage] Primary fetch failed:', error.message);
      try {
        if (isCategory) {
        ...
        console.log('[initializeCategoryPage] Alternate fetch succeeded:', categoryObj);
    ...
    console.log('[initializeCategoryPage] Normalized category object:', categoryObj);
    ...
    if (categoryTitleEl && displayTitle) {
      ...
      console.log('[initializeCategoryPage] Set title element to:', displayTitle);
    }

    const pageTitle = (categoryObj.metaTitle || categoryObj.name || '') + ' – Furnistør';
    ...
    try {
      ...
      if (ogDesc && (categoryObj.metaDescription || preferredDesc)) {
        ogDesc.setAttribute('content', categoryObj.metaDescription || preferredDesc);
      }
...
function runInitialization() {
  ...
  setTimeout(() => {
    ...
    // NEW: Load home products if on homepage
    if (typeof loadHomeProducts === 'function') {
      loadHomeProducts();
    }
  }, 100);
}
...
window.testCategoryFetch = async (slug = 'mirrors') => {
  try {
    ...
    console.error('[testCategoryFetch] Make sure backend server is running at', API_BASE_URL);
      ...
      <a href="${productLink}" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">
        ...
        <span class="price">
          <span class="woocommerce-Price-amount amount">
            <bdi>
              <span class="woocommerce-Price-currencySymbol">${product.currencySymbol || '$'}</span>
              ${product.price}
            </bdi>
          </span>
        </span>
...
console.log('[api-client.js] Script loaded successfully');

// Intercept mirrored external links to keep user on local clone
...
/**
 * Load random items for the Home Page "Explore products" grid
 */
async function loadHomeProducts() {
  const gridContainer = document.querySelector('.products.grid');
  if (!gridContainer) {
    console.log('[loadHomeProducts] No .products.grid found, skipping.');
    return;
  }

  // Only run on homepage
  const pathname = window.location.pathname;
  // Consider both root / and index.html
  if (pathname !== '/' && !pathname.endsWith('index.html')) {
    console.log('[loadHomeProducts] Not on homepage, skipping.');
    return;
  }

  console.log('[loadHomeProducts] Initializing grid hydration...');

  // Identify the template item (the one we cleaned up in index.html)
  const templateItem = gridContainer.querySelector('li.product');
  if (!templateItem) {
    console.error('[loadHomeProducts] No template li.product found in grid!');
    return;
  }

  // Clone the template deeply so we can replicate it
  const template = templateItem.cloneNode(true);

  // Clean up the template just in case
  const loader = template.querySelector('.loader');
  if (loader) loader.remove();

  // Remove the existing static item using innerHTML clear for simplicity/thoroughness
  // (We have the clone in 'template' variable now)
  ...
  try {
    ...
    // 2. Shuffle and Pick 12
    const shuffled = allProducts.sort(() => 0.5 - Math.random());
    const selectedProducts = shuffled.slice(0, 12);

    console.log(`[loadHomeProducts] Selected ${selectedProducts.length} random products.`);

    // 3. Render
    selectedProducts.forEach(product => {
      const clone = template.cloneNode(true);

      hydrateProductItem(clone, product);

      gridContainer.appendChild(clone);
    });

  } catch (err) {
    console.error('[loadHomeProducts] Failed to load products:', err);
    // Restore template if fail, with error message
    gridContainer.appendChild(template);
    const title = template.querySelector('.woocommerce-loop-product__title');
    if (title) title.textContent = "Error loading products";
  }
}

/**
 * Helper to populate a complex DOM template with product data
 */
function hydrateProductItem(domElement, product) {
  const productLink = `/product/${product.slug}`;

  // 1. Title
  // Selector based on index.html structure: h3 > a
  const titleLink = domElement.querySelector('h3.lb-element-woocommerce-product-title a') ||
    domElement.querySelector('.woocommerce-loop-product__title');
  ...
  // 2. Main Link (surrounding image)
  const imageLinks = domElement.querySelectorAll('.image-set__entry a');
  ...
  // 3. Images
  const entries = domElement.querySelectorAll('.image-set__entry');
  const img1Container = entries[0];
  const img2Container = entries[1];

  // Main Image
  if (img1Container) {
    const img = img1Container.querySelector('img');
    if (img && product.images && product.images[0]) {
  ...
  // Hover Image
  if (img2Container) {
    const img = img2Container.querySelector('img');
    const secondaryImage = (product.images && product.images[1]) ? product.images[1] : (product.images && product.images[0]);
    if (img && secondaryImage) {
      img.src = secondaryImage.src;
      img.srcset = '';
      img.alt = secondaryImage.alt || product.name;
    }
  }

  // 4. Price
  const priceContainer = domElement.querySelector('.price');
  if (priceContainer) {
    const symbol = product.currencySymbol || '$';
    if (product.originalPrice) {
      priceContainer.innerHTML = `
                <del aria-hidden="true"><span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">${symbol}</span>${product.originalPrice}</bdi></span></del> 
                <ins aria-hidden="true"><span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">${symbol}</span>${product.price}</bdi></span></ins>
             `;
    } else {
      priceContainer.innerHTML = `
                <span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">${symbol}</span>${product.price}</bdi></span>
             `;
    }
  }

  // 5. Sale Badge
  const saleBadgeContainer = domElement.querySelector('.lb-element-woocommerce-product-sale-badge');
  if (saleBadgeContainer) {
    if (product.originalPrice) {
      saleBadgeContainer.style.display = 'block';
    } else {
      saleBadgeContainer.style.display = 'none';
    }
  }

  // 6. View Product Button
  const addToCartBtn = domElement.querySelector('.add-to-cart');
  if (addToCartBtn) {
